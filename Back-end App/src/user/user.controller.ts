...
{
  try{
    const type = await em.findOne(UserType, {_id: req.body.sanitizedInput.userType}) as UserType// validar que exista tipo de usuario en un futuro
    
    const isEmailExist = await em.findOne(User, { email: req.body.sanitizedInput.email }) //validates that email its not registered
    if (isEmailExist) {
      return res.status(400).json({message:'Email already registered'})
      } else {
      
      const salt = await bcrypt.genSalt(10);
      const hashPassword = await bcrypt.hash(req.body.sanitizedInput.password, salt);
...
{
  try{
    const user = await em.findOne(User, {email: req.body.sanitizedInput.email}, {populate:['userType']}) //ver si este populate es correcto
    if (!user){    
      return res.status(404).json({message:'User Not Found'})
    }

    const isPasswordCorrect = await bcrypt.compare(req.body.sanitizedInput.password, user.password)
    if (!isPasswordCorrect){ 
      return res.status(400).json({message:'Password is incorrect'})
    }
    
    //create token
    const token = jwt.sign({_id : user._id, name : user.name, surname: user.surname, userType: user.userType}, secret) //token created
    return res.cookie('access_token', token, {httpOnly: true}) //send token to the user through the cookie
    .status(200)
    .json({
      status: 200,
...
{
  try{
    const type = await em.findOne(UserType, {description: 'admin'}) as UserType
