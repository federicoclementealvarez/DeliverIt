...
export class HomeDeliveryBoyComponent {
  ...
  ngOnInit() {
    this.orderService
      ...
      .subscribe((response) => (this.currentDeliveries = response));
    this.orderService
      .findAllByDelivery()
      .subscribe((response) => (this.pastDeliveries = response.slice(0, 3))); //shows the last 3 delivieries, ordered by dateTimeArrival DESC
  }
  ...
  setArrival(idOrder: string, ammountToUpdate: number) {
    this.orderService.setDateTimeArrival(idOrder).subscribe(() => {
      alert('Pedido entregado con éxito');
      this.updateUser(ammountToUpdate);
