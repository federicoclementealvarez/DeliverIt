...
export async function findAll(req: Request, res: Response)
{
  try 
  {
    ...
    return res.status(200).json({message:'found all withdrawals ', data: withdrawals})
  }
...
export async function findOne (req: Request, res:Response)
{
  try
  {
    ...
    const withdrawal = await em.findOne(Withdrawal,req.params.id,{populate:['user']})
    
    if(withdrawal===null){ return res.status(404).json({message: 'Withdrawal not found'})}
    
    return res.status(200).json({message: 'Withdrawal found', data: withdrawal})
  }
...
export async function add(req: Request, res: Response) 
{
  try
  {
    ...
    await em.flush()
    return res.status(201).json({ message: 'Withdrawal created successfully', data: withdrawal })
  }
